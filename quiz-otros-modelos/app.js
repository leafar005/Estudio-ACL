/**
 * QUIZ APP — Otros Modelos + Medición
...
(function () {
  ...
  function formatJustification(text) {
    return text
      ...
      .replace(/(ISO[\s\/](?:IEC\s)?[\d]{4,5}(?::[\d]{4})?)/g, '<strong>$1</strong>')
      .replace(/(SPICE|MMIS|ITmark|SwTQM|EFQM|AENOR|PathFinder|INCIBE|SQuaRE|CMMI[-\s]?(?:DEV)?)/gi, '<strong>$1</strong>')
      .replace(/\b(GQM|PSM|DRE|KPI|KPIs|LOC|KLOC|MTBF|MTTR|ROI|DoD|ESI|SEI)\b/g, '<strong>$1</strong>')
      .replace(/\b(SUM|MCV|GCS|RQU|RQSIS|PP|ECP|GC|MED|ACS)\b/g, '<strong>$1</strong>');
  }
